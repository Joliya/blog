...
func Index(c *gin.Context) {
	...
	if len(keyword) > 0 {
		...
		postIds, err := dao.GetPostIdsByContent(keyword)
		if err == nil {
			params.Ids["ids"] = postIds
		}
		categoryIds, err := dao.GetCategoryIdsByName(keyword)
		if err == nil {
			params.Ids["category_ids"] = categoryIds
		}
		tagIds, err := dao.GetTagIdsByName(keyword)
		if err == nil {
			params.Ids["tag_ids"] = tagIds
		}
	}

	posts, err := dao.GetPosts(params)
	if err != nil {
...
func PostInfo(c *gin.Context) {
	id := c.Query("id")
	post := dao.GetPost(id)
	category := dao.GetCategory(post.CategoryId)
	allTags, _ := dao.GetTags()
	tagIds := post.TagIds
